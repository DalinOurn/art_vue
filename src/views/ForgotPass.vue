...
export default {
    ...
    methods:{
        submitEmail() {
            this.$http.post('http://localhsot:3001/auth/sendMail', {code: this.code, email: this.email, newPass: this.newPass}, {withCredentials: true, headers: {"Content-type": "application/json",}})
             .then((res) => {
                if (res.data.success) {
                    this.error = "";
        ...
        submitPass() {
            this.$http.post('http://localhsot:3001/auth/newPassword', {code: this.code, email: this.email, newPass: this.newPass}, {withCredentials: true, headers: {"Content-type": "application/json",}})
             .then((res) => {
                 console.log(res);
                if (res.data.success) {
